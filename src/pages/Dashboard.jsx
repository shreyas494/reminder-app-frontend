...
export default function Dashboard() {
  ...
  /* 🔑 FIXED: backend returns { data, page, totalPages } */
  const fetchReminders = async () => {
    const res = await API.get("/reminders");
    setReminders(res.data.data); // ✅ THIS WAS THE BUG
  };
...
function Td({ children }) {
  return <td className="p-3 text-gray-700 dark:text-gray-300">{children}</td>;
}
...
function ActionButton({ children, onClick, color }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-lg text-xs ${colors[color]}`}
    >
      {children}
    </button>
  );
